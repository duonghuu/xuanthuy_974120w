...
function get_item(){
	...
	if(!$id)
		transfer("Không nhận được dữ liệu", "index.php?com=video&act=man".$urlcu);
	
	$sql = "select * from #_video where id='".$id."'";
	$d->query($sql);
	if($d->num_rows()==0) transfer("Dữ liệu không có thực", "index.php?com=video&act=man".$urlcu);
	$item = $d->fetch_array();
}
//===========================================================
function save_item(){
	...
	if($id){
		...
		if($photo = upload_image("file", _format_duoihinh ,_upload_khac,$file_name)){
			$data['photo'] = $photo;
			$row=get_fetch("select photo from #_video where id=".$id);
			if($row){
		...
		if($d->update($data))			
			...
			transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=video&act=man".$urlcu);
	}else{
		...
		if($d->insert($data))
			...
			transfer("Lưu dữ liệu bị lỗi", "index.php?com=video&act=man".$urlcu);
	}
}
//===========================================================
function delete_item(){
	...
	if(isset($_GET['id'])){
		...
		if($d->query($sql))
			...
			transfer("Xóa dữ liệu bị lỗi", "index.php?com=video&act=man".$urlcu);
		...
		} redirect("index.php?com=video&act=man".$urlcu);} else transfer("Không nhận được dữ liệu", "index.php?com=video&act=man".$urlcu);
}
