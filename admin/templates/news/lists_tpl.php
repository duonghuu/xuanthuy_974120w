...
<div class="control_frm" style="margin-top:25px;">
    <div class="bc">
        <ul id="breadcrumbs" class="breadcrumbs">
        	<li><a href="index.php?com=news&act=man_list<?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>"><span>Danh mục cấp 2</span></a></li>
        	<?php if($_GET['key']!=''){ ?>
...
<div class="control_frm" style="margin-top:0;">
  	<div style="float:left;">
    	<input type="button" class="blueB" value="Thêm" onclick="location.href='index.php?com=news&act=add_list<?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>'" />
   ...
   <tr>
        ...
        $item_danhmuc = get_fetch("select ten from table_news_danhmuc where id='".$items[$i]['id_danhmuc']."'");
   		...
   		<td class="title_name_data">
            <a href="index.php?com=news&act=edit_list&id_danhmuc=<?=$items[$i]['id_danhmuc']?><?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>&id=<?=$items[$i]['id']?><?php if($_REQUEST['p']!='') echo'&p='.$_REQUEST['p'];?>" class="tipS SC_bold"><?=$items[$i]['ten']?></a>
        </td>
			<?php } ?>

			<?php if(in_array('hinhanh',$config['type'])) { ?>
        <td align="center">
            <a href="index.php?com=news&act=edit_list&id_danhmuc=<?=$items[$i]['id_danhmuc']?><?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>&id=<?=$items[$i]['id']?><?php if($_REQUEST['p']!='') echo'&p='.$_REQUEST['p'];?>" class="tipS SC_bold"><img src="<?=_upload_tintuc.$items[$i]['thumb']?>" style="max-height:40px; max-width:70px;" /></a>
        </td>
			<?php } ?>
