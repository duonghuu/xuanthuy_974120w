   ...
   <form name="frm" method="post" action="index.php?com=news&act=savestt_danhmuc&type=<?=$_REQUEST['type']?><?php if($_REQUEST['p']!='') echo'&p='.$_REQUEST['p'];?>" enctype="multipart/form-data" class="nhaplieu">
     <?php for($i=0, $count=count($items); $i<$count; $i++){?>
       <tr>
        ...
        <?php if(in_array('hinhanh',$config['type'])) { ?>
          <td align="center">
            <a href="index.php?com=news&act=edit_danhmuc&id=<?=$items[$i]['id']?>&type=<?=$_REQUEST['type']?><?php if($_REQUEST['p']!='') echo'&p='.$_REQUEST['p'];?>" class="tipS SC_bold"><img src="<?=_upload_tintuc.$items[$i]['thumb']?>" style="max-height:40px; max-width:70px;" /></a>
          </td>
        <?php } ?>
        <?php if(in_array('noibat',$config['type'])) { ?>
          <td align="center" >
