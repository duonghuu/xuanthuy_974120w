...
function get_main_list()
    ...
    if(!empty($_REQUEST['id_danhmuc'])){
        $getdata = get_result("select * from table_news_list where 
            id_danhmuc=".$_REQUEST['id_danhmuc']." order by stt");        
    }
    $str='
...
<div class="control_frm" style="margin-top:25px;">
    <div class="bc">
        <ul id="breadcrumbs" class="breadcrumbs">
        	<li><a href="index.php?com=news&act=man_item<?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>"><span>Danh mục cấp 4</span></a></li>
        	<?php if($_GET['key']!=''){ ?>
...
<div class="control_frm" style="margin-top:0;">
  	<div style="float:left;">
    	<input type="button" class="blueB" value="Thêm" onclick="location.href='index.php?com=news&act=add_item<?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>'" />
...
<div class="widget">
  ...
  <table cellpadding="0" cellspacing="0" width="100%" class="sTable withCheck mTable" id="checkAll">
   ...
   <tr>
        ...
        <td align="center">
       	 	<?php
				$sql_list="select ten from table_news_list where id='".$items[$i]['id_list']."'";
        ...
        <td align="center">
   		...
   		<td class="title_name_data">
            <a href="index.php?com=news&act=edit_item&id=<?=$items[$i]['id']?>&id_danhmuc=<?=$items[$i]['id_danhmuc']?>&id_list=<?=$items[$i]['id_list']?>&id_cat=<?=$items[$i]['id_cat']?><?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?><?php if($_REQUEST['p']!='') echo'&p='.$_REQUEST['p'];?>" class="tipS SC_bold"><?=$items[$i]['ten']?></a>
        </td>
			<?php } ?>
