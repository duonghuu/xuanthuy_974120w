 ...
 <table cellpadding="0" cellspacing="0" width="100%" class="sTable withCheck mTable" id="checkAll">
    <thead>
      <tr>
        ...
        <td class="sortCol"><div>Chủ đề<span></span></div></td>
	...
	<tbody>
     <?php for($i=0, $count=count($items); $i<$count; $i++){?>
        ...
        <td>
            <a href="index.php?com=lienhe&act=edit&id=<?=$items[$i]['id']?><?php if($_REQUEST['p']!='') echo'&p='.$_REQUEST['p'];?>&type=<?=$_REQUEST['type']?>" class="tipS SC_bold">
             <?php if($items[$i]['ten']!='')echo $items[$i]['ten'];else echo $items[$i]['tenen'];?>
            </a>
        </td>
        
         <td class="title_name_data">
            <a href="index.php?com=lienhe&act=edit&id=<?=$items[$i]['id']?><?php if($_REQUEST['p']!='') echo'&p='.$_REQUEST['p'];?>&type=<?=$_REQUEST['type']?>" class="tipS SC_bold"><?php if($items[$i]['chude']!='')echo $items[$i]['chude'];else echo $items[$i]['chudeen'];?></a>
        </td>
        ...
        <td class="actBtns">
            <a href="index.php?com=lienhe&act=edit&id=<?=$items[$i]['id']?><?php if($_REQUEST['p']!='') echo'&p='.$_REQUEST['p'];?>&type=<?=$_REQUEST['type']?>" title="" class="smallButton tipS" original-title="Sửa danh mục"><img src="./images/icons/dark/pencil.png" alt=""></a>
           <a href="index.php?com=lienhe&act=delete&id=<?=$items[$i]['id']?><?php if($_REQUEST['p']!='') echo'&p='.$_REQUEST['p'];?>&type=<?=$_REQUEST['type']?>" onClick="if(!confirm('Xác nhận xóa <?=$items[$i]['ten']?>')) return false;" title="" class="smallButton tipS" original-title="Xóa bài viết"><img src="./images/icons/dark/close.png" alt=""></a> 
        </td>
      </tr>
           <?php } ?> 
    </tbody>
