...
<script language="javascript">
    ...
    {   
        ...
        window.location ="index.php?com=place&act=man_street&key="+a;
        return true;
    }                   
</script>
<?php
...
{
    $getdata = get_result("select * from table_place_ward where id_dist=".$_REQUEST['id_dist']." order by stt,id desc");
    $str='
...
<tbody>
   <?php for($i=0, $count=count($items); $i<$count; $i++){?>
    ...
    <td class="title_name_data">
        <a href="index.php?com=place&act=edit_street&id=<?=$items[$i]['id']?>&id_city=<?=$items[$i]['id_city']?>&id_dist=<?=$items[$i]['id_dist']?>&id_ward=<?=$items[$i]['id_ward']?>" class="tipS SC_bold"><?=$items[$i]['ten']?></a>
    </td>
    <td align="center">
    ...
    <td class="actBtns">
        <a href="index.php?com=place&act=edit_street&id=<?=$items[$i]['id']?>&id_city=<?=$items[$i]['id_city']?>&id_dist=<?=$items[$i]['id_dist']?>&id_ward=<?=$items[$i]['id_ward']?>" title="" class="smallButton tipS" original-title="Sửa"><img src="./images/icons/dark/pencil.png" alt=""></a>
            <a href="" onclick="CheckDelete('index.php?com=place&act=delete_street&id=<?=$items[$i]['id']?>&id_city=<?=$items[$i]['id_city']?>&id_dist=<?=$items["id_dist"]?>&id_ward=<?=$items[$i]['id_ward']?>'); return false;" title="" class="smallButton tipS" original-title="Xóa"><img src="./images/icons/dark/close.png" alt=""></a>        </td>
      </tr>
         <?php } ?>
                </tbody>
  </table>
