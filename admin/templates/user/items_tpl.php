    ...
    <tbody>
     <?php for($i=0, $count=count($items); $i<$count; $i++){?>
      ...
      <td class="title_name_data">
        <a href="index.php?com=user&act=edit&id_list=<?=$items[$i]['id_list']?>&id_cat=<?=$items[$i]['id_cat']?>&id=<?=$items[$i]['id']?><?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>" class="tipS SC_bold"><?=$items[$i]['ten']?></a>
      </td>
