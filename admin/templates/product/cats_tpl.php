        ...
        $item_danhmuc = get_fetch("select ten from table_product_danhmuc where id='".$items[$i]['id_danhmuc']."'");
         ...
         <td align="center">
   		...
   		<td class="title_name_data">
			...
			<?php if(in_array('hinhanh',$config['type'])) { ?>
        <td align="center">
            <a href="index.php?com=product&act=edit_cat&id=<?=$items[$i]['id']?>&id_danhmuc=<?=$items[$i]['id_danhmuc']?>&id_list=<?=$items[$i]['id_list']?><?php if($_REQUEST['type']!='') echo'&type='.$_REQUEST['type'];?><?php if($_REQUEST['p']!='') echo'&p='.$_REQUEST['p'];?>" class="tipS SC_bold"><img src="<?=_upload_sanpham.$items[$i]['thumb']?>" style="max-height:40px; max-width:70px;" /></a>
        </td>
			<?php } ?>
